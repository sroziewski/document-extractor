...
import com.opi.lil.actors.CheckerProtocol
import com.opi.lil.actors.CheckerProtocol.{WARCMessage}
...
class StandaloneParser {
  ...
  def parse(url: String) = {
    ...
    val fs: List[Future[(WARCMessage, Status)]] = transform(it){
      ...
      lines => Future { (CheckerProtocol.buildWARCMessage(lines), Success()) }
    }
